...
/// 'libc' functions.
int _openFd(String path, int flags, {ffi.Allocator allocator = ffi.calloc}) {
  final pathAddr = Utils.toUtf8(path);
  try {
    return libc.open(pathAddr, flags, 0);
  } finally {
...
class NamedPipe {
  ...
  void createSync({int mode = 0x1FF, ffi.Allocator allocator = ffi.calloc}) {
    final pathAddr = Utils.toUtf8(path);
    try {
      final result = libc.mkfifo(pathAddr, mode);
      if (result < 0) {
  ...
  /// operations.
  void deleteSync({ffi.Allocator allocator = ffi.calloc}) {
    if (!existsSync()) {
      return;
    }
    final pathAddr = Utils.toUtf8(path);
    try {
      final result = libc.unlink(pathAddr);
      if (result < 0) {
  ...
  Stream<List<int>> openRead({ffi.Allocator allocator = ffi.calloc}) {
...
class _NamedPipeWriter implements Sink<List<int>>, StreamConsumer<List<int>> {
  ...
  Future<void> _add(Future<void> waitedFuture, List<int> data, {ffi.Allocator allocator = ffi.calloc}) async {
    ...
    final pointer = allocator<ffi.Uint8>(length);
    ...
    while (true) {
      ...
      if (fd <= 0) {
        ...
        if (libc.errorCode == libc.ENXIO) {
          // Wait a bit more
          await Future.delayed(_shortDuration);
          if (_timeout != null && DateTime.now().isAfter(startedAt.add(_timeout))) {
