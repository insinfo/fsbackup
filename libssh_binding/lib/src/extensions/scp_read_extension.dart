...
extension ScpReadExtension on Libssh {
  ...
  String scpReadFileAsString(ssh_session session, String fullPath) {
    ...
    if (rc != SSH_OK) {
      ssh_scp_free(scp);
      throw Exception("Error initializing scp session: ${ssh_get_error(session.cast()).cast<Utf8>()}");
    }

    rc = ssh_scp_pull_request(scp);
    if (rc != ssh_scp_request_types.SSH_SCP_REQUEST_NEWFILE) {
      throw Exception("Error receiving information about file: ${ssh_get_error(session.cast()).cast<Utf8>()}");
    }
    ...
    var bufferLength = 256, nbytes = 0;
    final buffer = malloc<Int8>(bufferLength);
    var receive = '';
    ...
    while (true) {
      ...
      if (nbytes < 0 || nbytes == SSH_ERROR || nbytes == fileSize) {
        ssh_scp_close(scp);
        ssh_scp_free(scp);
        malloc.free(buffer);
        break;
