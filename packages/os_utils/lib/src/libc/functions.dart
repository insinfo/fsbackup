...
final chmod = libraryLoader.open().lookupFunction<_chmod_C, _chmod_Dart>('chmod');

final close = libraryLoader.open().lookupFunction<_close_C, _close_Dart>('close');
...
final mkfifo = libraryLoader.open().lookupFunction<_mkfifo_C, _mkfifo_Dart>('mkfifo');

final mkdir = libraryLoader.open().lookupFunction<_mkdir_C, _mkdir_Dart>('mkdir');

final mmap = libraryLoader.open().lookupFunction<_mmap_C, _mmap_Dart>('mmap');

final mprotect = libraryLoader.open().lookupFunction<_mprotect_C, _mprotect_Dart>('mprotect');

final munmap = libraryLoader.open().lookupFunction<_munmap_C, _munmap_Dart>('munmap');

final open = libraryLoader.open().lookupFunction<_open_C, _open_Dart>('open');

final poll = libraryLoader.open().lookupFunction<_poll_C, _poll_Dart>('poll');

final read = libraryLoader.open().lookupFunction<_read_C, _read_Dart>('read');

final unlink = libraryLoader.open().lookupFunction<_unlink_C, _unlink_Dart>('unlink');

final write = libraryLoader.open().lookupFunction<_write_C, _write_Dart>('write');

typedef _chmod_C = ffi.Int32 Function(ffi.Pointer<ffi.Utf8> name, ffi.Int32 mode);

typedef _chmod_Dart = int Function(ffi.Pointer<ffi.Utf8> name, int mode);

typedef _close_C = ffi.Int32 Function(ffi.Int32 fd);

typedef _close_Dart = int Function(int fd);

typedef _mkfifo_C = ffi.Int32 Function(ffi.Pointer<ffi.Utf8> name, ffi.Int32 mode);

typedef _mkfifo_Dart = int Function(ffi.Pointer<ffi.Utf8> name, int mode);
